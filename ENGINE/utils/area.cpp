...
// Interactive editor moved to ui/area_ui.{hpp,cpp}
...
{
    AreaUI::Result out;
    if (!AreaUI::edit_over_area(renderer, base, window_w, window_h, out) || out.points.empty()) {
        throw std::runtime_error("[Area: editor] No points drawn");
    }
    int pivot_x = out.bg_w / 2;
    int pivot_y = out.bg_h;
    points = out.points;
    pos_X = pivot_x;
    pos_Y = pivot_y;
    update_geometry_data();
...
{
    AreaUI::Result out;
    if (!AreaUI::edit_over_texture(renderer, background, window_w, window_h, out) || out.points.empty()) {
        throw std::runtime_error("[Area: editor] No points drawn");
    }
    int pivot_x = out.bg_w / 2;
    int pivot_y = out.bg_h;
...
void Area::edit_with_ui(SDL_Renderer* renderer, int window_w, int window_h) {
    AreaUI::Result out;
    if (!AreaUI::edit_over_area(renderer, *this, window_w, window_h, out) || out.points.empty()) {
        throw std::runtime_error("[Area: editor] No points drawn");
    }
    const int pivot_x = out.bg_w / 2;
    const int pivot_y = out.bg_h;
    points = out.points;
    pos_X = pivot_x;
    pos_Y = pivot_y;
    update_geometry_data();
...
void Area::edit_with_ui(SDL_Texture* background, SDL_Renderer* renderer, int window_w, int window_h) {
    AreaUI::Result out;
    if (!AreaUI::edit_over_texture(renderer, background, window_w, window_h, out) || out.points.empty()) {
        throw std::runtime_error("[Area: editor] No points drawn");
    }
