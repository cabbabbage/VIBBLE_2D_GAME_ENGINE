...
SDL_Point Parallax::apply(int ax, int ay) const {
    if (disabled_) {
        // No parallax, just return centered relative to last reference
        return { ax - lastPx_ + static_cast<int>(halfWidth_),
                 ay - lastPy_ + static_cast<int>(halfHeight_) };
    }
...
SDL_Point Parallax::inverse(int screen_x, int screen_y) const {
    if (disabled_) {
        // Reverse the simple centered transform
        int world_x = lastPx_ + screen_x - static_cast<int>(halfWidth_);
        int world_y = lastPy_ + screen_y - static_cast<int>(halfHeight_);
