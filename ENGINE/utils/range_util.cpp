...
bool Range::xy(const SDL_FPoint& p, double& x, double& y) {
 x = static_cast<double>(p.x);
  y = static_cast<double>(p.y);
   return true;
}
...
bool Range::is_in_range(const Asset* a, const SDL_FPoint& b, int radius) {
 double ax, ay, bx, by;
  if (!xy(a, ax, ay) || !xy(b, bx, by)) return false;
   return in_range_xy(ax, ay, bx, by, radius);
}
...
bool Range::is_in_range(const SDL_FPoint& a, const Asset* b, int radius) {
 double ax, ay, bx, by;
  if (!xy(a, ax, ay) || !xy(b, bx, by)) return false;
   return in_range_xy(ax, ay, bx, by, radius);
}
...
bool Range::is_in_range(const SDL_Point& a, const SDL_FPoint& b, int radius) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return in_range_xy(ax, ay, bx, by, radius);
...
bool Range::is_in_range(const SDL_FPoint& a, const SDL_Point& b, int radius) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return in_range_xy(ax, ay, bx, by, radius);
...
bool Range::is_in_range(const SDL_FPoint& a, const SDL_FPoint& b, int radius) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return in_range_xy(ax, ay, bx, by, radius);
...
double Range::get_distance(const Asset* a, const SDL_FPoint& b) {
 double ax, ay, bx, by;
  if (!xy(a, ax, ay) || !xy(b, bx, by)) return std::numeric_limits<double>::infinity();
   return distance_xy(ax, ay, bx, by);
}
...
double Range::get_distance(const SDL_FPoint& a, const Asset* b) {
 double ax, ay, bx, by;
  if (!xy(a, ax, ay) || !xy(b, bx, by)) return std::numeric_limits<double>::infinity();
   return distance_xy(ax, ay, bx, by);
}
...
double Range::get_distance(const SDL_Point& a, const SDL_FPoint& b) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return distance_xy(ax, ay, bx, by);
...
double Range::get_distance(const SDL_FPoint& a, const SDL_Point& b) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return distance_xy(ax, ay, bx, by);
...
double Range::get_distance(const SDL_FPoint& a, const SDL_FPoint& b) {
 double ax, ay, bx, by;
  xy(a, ax, ay);
   xy(b, bx, by);
    return distance_xy(ax, ay, bx, by);
...
std::vector<Asset*> Range::get_in_range(const Asset* center,
                                        int radius,
                                        const std::vector<Asset*>& candidates) {
 std::vector<Asset*> out;
  out.reserve(candidates.size());
   get_in_range(center, radius, candidates, out);
    out.shrink_to_fit();
     return out;
}

std::vector<Asset*> Range::get_in_range(const SDL_Point& center,
                                        int radius,
                                        const std::vector<Asset*>& candidates) {
 std::vector<Asset*> out;
  out.reserve(candidates.size());
...
std::vector<Asset*> Range::get_in_range(const SDL_FPoint& center,
                                        int radius,
                                        const std::vector<Asset*>& candidates) {
 std::vector<Asset*> out;
  out.reserve(candidates.size());
...
void Range::get_in_range(const Asset* center,
                         int radius,
                         const std::vector<Asset*>& candidates,
                         std::vector<Asset*>& out) {
 double cx, cy;
  out.clear();
   if (!xy(center, cx, cy)) return;
    const double r2 = static_cast<double>(radius) * static_cast<double>(radius);
...
void Range::get_in_range(const SDL_FPoint& center,
                         int radius,
                         const std::vector<Asset*>& candidates,
                         std::vector<Asset*>& out) {
 double cx, cy;
