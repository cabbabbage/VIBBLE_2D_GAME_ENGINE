...
public:
    ...
    void set_asset_paths(const std::string& asset_dir_path, const std::string& info_json_path);
    void open();
    void close();
    bool is_open() const;
...
private:
    ...
    struct AnimUI {
        ...
        std::unique_ptr<DMSlider>    alpha_sl;
        std::unique_ptr<DMButton>    compute_btn;
        std::unique_ptr<DMButton>    crop_btn;
        std::unique_ptr<DMTextBox>   crop_summary;
