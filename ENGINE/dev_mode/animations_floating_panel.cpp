...
void AnimationsFloatingPanel::open() {
  if (box_)
    box_->set_visible(true);
}
void AnimationsFloatingPanel::close() {
  if (box_)
    box_->set_visible(false);
}
bool AnimationsFloatingPanel::is_open() const {
  return box_ && box_->is_visible();
}

void AnimationsFloatingPanel::update(const Input &input, int screen_w,
                                     int screen_h) {
  if (box_)
    box_->update(input, screen_w, screen_h);
}
...
bool AnimationsFloatingPanel::handle_event(const SDL_Event &e) {
  ...
  // Header: new button
  if (new_btn_) {
    // ButtonWidget already toggles click; here we act on value state by
    // inspecting handle_event result is not trivial. Instead, we rebuild rows
    // on each frame and wire ButtonWidget with on_click callback in
    // rebuild_rows().
  }
  ...
  for (auto &it : items_) {
    ...
    nlohmann::json payload = it->last_payload.is_object()
                                 ? it->last_payload
                                 : nlohmann::json::object();
    if (!payload.is_object())
      payload = nlohmann::json::object();
    nlohmann::json src =
        payload.contains("source") && payload["source"].is_object()
            ? payload["source"]
            : nlohmann::json::object();
    // kind
    std::string kind = "folder";
    // DMDropdown has no direct API to set label; we track selection only via
    // index. We reconstruct from UI by reading the displayed options index via
    // render-only approach is not feasible here. As a workaround, we encode
    // kind selection in the textbox path/ref visibility: if ref_dd exists and
    // has options, and path_box value empty -> animation.
    if (it->kind_dd) {
    ...
    if (kind == "folder") {
      ...
      std::string ref =
          (!names.empty() && it->ref_dd)
              ? names[clampi(it->ref_dd->selected(), 0, (int)names.size() - 1)]
              : std::string{};
      src["name"] = ref;
      src["path"] = "";
    }
    payload["source"] = src;
    // flags
    payload["flipped_source"] =
        it->flipped_cb ? it->flipped_cb->value() : false;
    payload["reverse_source"] =
        it->reversed_cb ? it->reversed_cb->value() : false;
...
void AnimationsFloatingPanel::render(SDL_Renderer *r, int screen_w,
                                     int screen_h) const {
  if (box_)
    box_->render(r);
}
...
std::vector<std::string> AnimationsFloatingPanel::current_names_sorted() const {
  ...
  if (info_)
    names = info_->animation_names();
  std::sort(names.begin(), names.end());
  return names;
}

int AnimationsFloatingPanel::compute_frames_from_source(
    const AssetInfo &info, const nlohmann::json &source) {
  try {
    if (!source.is_object())
      return 1;
    ...
    if (kind == "animation") {
      ...
      if (!other.is_object())
        return 1;
      return compute_frames_from_source(
          info, other.value("source", nlohmann::json::object()));
    }
    // folder
...
nlohmann::json
AnimationsFloatingPanel::default_payload(const std::string &name) {
  nlohmann::json p;
  p["source"] = nlohmann::json::object(
      {{"kind", "folder"}, {"path", name}, {"name", nullptr}});
...
void AnimationsFloatingPanel::rebuild_header_row() {
  ...
  for (size_t i = 0; i < names.size(); ++i)
    if (names[i] == info_->start_animation) {
      sel = (int)i;
      break;
    }
  ...
  new_btn_ = std::make_unique<DMButton>(
      "New Animation", &DMStyles::CreateButton(), 160, DMButton::height());
  header_widgets_.push_back(
      std::make_unique<ButtonWidget>(new_btn_.get(), [this]() {
        if (!info_)
          return;
        ...
        while (exists(nm)) {
          nm = base + std::string("_") + std::to_string(i++);
        }
...
void AnimationsFloatingPanel::rebuild_animation_rows() {
  ...
  for (const auto &nm : names) {
    ...
    if (!ui->last_payload.is_object())
      ui->last_payload = nlohmann::json::object();
    nlohmann::json src =
        ui->last_payload.value("source", nlohmann::json::object());
    ...
    ui->del_btn = std::make_unique<DMButton>(
        "Delete", &DMStyles::DeleteButton(), 100, DMButton::height());
    ...
    ui->flipped_cb = std::make_unique<DMCheckbox>(
        "flipped", ui->last_payload.value("flipped_source", false));
    ui->reversed_cb = std::make_unique<DMCheckbox>(
        "reverse", ui->last_payload.value("reverse_source", false));
    ui->locked_cb = std::make_unique<DMCheckbox>(
        "locked", ui->last_payload.value("locked", false));
    ui->rnd_start_cb = std::make_unique<DMCheckbox>(
        "rnd start", ui->last_payload.value("rnd_start", false));
    ui->loop_cb = std::make_unique<DMCheckbox>(
        "loop", ui->last_payload.value("loop", false));

    int spd = 1;
    try {
      spd = (int)std::lround(ui->last_payload.value("speed_factor", 1.0f));
    } catch (...) {
      spd = 1;
    }
    if (spd == 0)
      spd = 1;
    spd = clampi(spd, -20, 20);
    ui->speed_sl = std::make_unique<DMSlider>("speed", -20, 20, spd);

    ui->movement_btn = std::make_unique<DMButton>(
        "Edit Movement...", &DMStyles::HeaderButton(), 180, DMButton::height());
    ...
    // Row A: ID + Delete
    ui->row_widgets.push_back(
        std::make_unique<TextBoxWidget>(ui->id_box.get()));
    ui->row_widgets.push_back(
        std::make_unique<ButtonWidget>(ui->del_btn.get(), [this, nm]() {
          if (!info_)
            return;
          info_->remove_animation(nm);
          info_->update_info_json();
          rebuild_rows();
        }));
    rows_.push_back(std::vector<Widget *>{ui->row_widgets[0].get(),
                                          ui->row_widgets[1].get()});

    // Row B: Kind + Path/Ref
    ui->row_widgets.push_back(
        std::make_unique<DropdownWidget>(ui->kind_dd.get()));
    if (kind_idx == 0) {
      ui->row_widgets.push_back(
          std::make_unique<TextBoxWidget>(ui->path_box.get()));
    } else {
      ui->row_widgets.push_back(
          std::make_unique<DropdownWidget>(ui->ref_dd.get()));
    }
    rows_.push_back(std::vector<Widget *>{ui->row_widgets[2].get(),
                                          ui->row_widgets[3].get()});

    // Row C: flags
    ui->row_widgets.push_back(
        std::make_unique<CheckboxWidget>(ui->flipped_cb.get()));
    ui->row_widgets.push_back(
        std::make_unique<CheckboxWidget>(ui->reversed_cb.get()));
    ui->row_widgets.push_back(
        std::make_unique<CheckboxWidget>(ui->locked_cb.get()));
    ui->row_widgets.push_back(
        std::make_unique<CheckboxWidget>(ui->rnd_start_cb.get()));
    ui->row_widgets.push_back(
        std::make_unique<CheckboxWidget>(ui->loop_cb.get()));
    rows_.push_back(std::vector<Widget *>{
        ui->row_widgets[4].get(), ui->row_widgets[5].get(),
        ui->row_widgets[6].get(), ui->row_widgets[7].get(),
        ui->row_widgets[8].get()});

    // Row D: speed + movement + frames label
    ui->row_widgets.push_back(
        std::make_unique<SliderWidget>(ui->speed_sl.get()));
    ui->row_widgets.push_back(
        std::make_unique<ButtonWidget>(ui->movement_btn.get(), [this, nm]() {
          // TODO: hook movement modal overlay in C++
          (void)nm;
        }));
    ui->row_widgets.push_back(
        std::make_unique<TextBoxWidget>(ui->frames_label.get()));
    rows_.push_back(std::vector<Widget *>{ui->row_widgets[9].get(),
                                          ui->row_widgets[10].get(),
                                          ui->row_widgets[11].get()});
