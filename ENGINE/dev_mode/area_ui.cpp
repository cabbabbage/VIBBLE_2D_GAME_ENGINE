...
namespace {
		...
		// helper to draw a labeled UI button
		static void ae_render_button(SDL_Renderer* rend, TTF_Font* font, const SDL_Rect& rct, SDL_Color col, const char* label) {
			...
			if (font && label) {
				...
				if (surf) {
					...
					if (tex) {
						...
						SDL_Rect tdst{ rct.x + (rct.w - tw)/2, rct.y + (rct.h - th)/2, tw, th };
						SDL_RenderCopy(rend, tex, nullptr, &tdst);
    ...
    // New: in-place overlay editor running on the provided renderer.
    static EditorResult run_area_editor_overlay(SDL_Texture* background,
                                                SDL_Renderer* r,
                                                int /*window_w*/, int /*window_h*/,
                                                const Area* initial_area = nullptr) {
        if (!background || !r) throw std::runtime_error("[Area Editor] Missing background or renderer");
        int tex_w = 0, tex_h = 0; SDL_QueryTexture(background, nullptr, nullptr, &tex_w, &tex_h);
        ...
        SDL_Rect prev_vp; SDL_RenderGetViewport(r, &prev_vp);
        SDL_Rect prev_clip; SDL_RenderGetClipRect(r, &prev_clip);
        float prev_sx = 1.0f, prev_sy = 1.0f; SDL_RenderGetScale(r, &prev_sx, &prev_sy);
        int prev_lw = 0, prev_lh = 0; SDL_RenderGetLogicalSize(r, &prev_lw, &prev_lh);
        SDL_bool prev_integer = SDL_RenderGetIntegerScale(r);

        int win_w = 0, win_h = 0; SDL_GetRendererOutputSize(r, &win_w, &win_h);
        if (win_w <= 0 || win_h <= 0) { win_w = tex_w; win_h = tex_h; }

        // Prepare drawing assets
        SDL_Surface* mask = SDL_CreateRGBSurfaceWithFormat(0, tex_w, tex_h, 32, SDL_PIXELFORMAT_RGBA32);
        if (!mask) throw std::runtime_error("[Area Editor] Failed to allocate mask surface");
        SDL_FillRect(mask, nullptr, SDL_MapRGBA(mask->format, 255, 0, 0, 0));
        SDL_Texture* mask_tex = SDL_CreateTexture(r, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, tex_w, tex_h);
        if (!mask_tex) { SDL_FreeSurface(mask); throw std::runtime_error("[Area Editor] Failed to create mask texture"); }
        SDL_SetTextureBlendMode(mask_tex, SDL_BLENDMODE_BLEND);

        auto upload_mask = [&]() {
            SDL_UpdateTexture(mask_tex, nullptr, mask->pixels, mask->pitch);
        };
        upload_mask();

        const double fit_scale = std::min(static_cast<double>(win_w) / tex_w, static_cast<double>(win_h) / tex_h);
        double camera_scale = fit_scale;
        int pan_x = 0, pan_y = 0;
        ...
        int brush = 10; bool drawing=false, erasing=false, panning=false; int last_mx=0,last_my=0; bool quit=false; bool draw_mode=true;

        bool ttf_inited_here = false; if (TTF_WasInit() == 0) { if (TTF_Init() == 0) ttf_inited_here = true; }
        TTF_Font* font = nullptr; auto open_font = [&]() -> TTF_Font* {
            TTF_Font* f = TTF_OpenFont("C:/Windows/Fonts/consola.ttf", 16); if (!f) f = TTF_OpenFont("C:/Windows/Fonts/arial.ttf", 16); return f;
        }; font = open_font();
        ...
        while (!quit) {
            SDL_Event e; while (SDL_PollEvent(&e)) {
                if (e.type == SDL_QUIT) { quit = true; }
                    ...
                    else if (e.key.keysym.sym == SDLK_c) { SDL_FillRect(mask, nullptr, SDL_MapRGBA(mask->format, 255, 0, 0, 0)); upload_mask(); }
                    else if (e.key.keysym.sym == SDLK_f) { camera_scale = fit_scale; pan_x = pan_y = 0; recompute_layout(draw_w, draw_h, off_x, off_y); }
                    else if (e.key.keysym.sym == SDLK_1) { camera_scale = 1.0; pan_x = pan_y = 0; recompute_layout(draw_w, draw_h, off_x, off_y); }
                } else if (e.type == SDL_MOUSEBUTTONDOWN) {
                    SDL_Rect btn_draw{10,10,100,30}; SDL_Rect btn_erase{120,10,100,30}; SDL_Rect btn_save{win_w-120,10,100,30}; SDL_Point mp{ e.button.x, e.button.y };
                    ...
                    else { if (e.button.button == SDL_BUTTON_LEFT) drawing = true; if (e.button.button == SDL_BUTTON_RIGHT) erasing = true; if (e.button.button == SDL_BUTTON_MIDDLE) { panning=true; last_mx=e.button.x; last_my=e.button.y; } }
                } else if (e.type == SDL_MOUSEBUTTONUP) {
                    if (e.button.button == SDL_BUTTON_LEFT) drawing = false; if (e.button.button == SDL_BUTTON_RIGHT) erasing = false; if (e.button.button == SDL_BUTTON_MIDDLE) panning = false;
                } else if (e.type == SDL_MOUSEWHEEL) {
                    int mx, my; SDL_GetMouseState(&mx, &my); double tx = (mx - off_x) / camera_scale; double ty = (my - off_y) / camera_scale;
                    if (e.wheel.y > 0) camera_scale = std::min(10.0, camera_scale * 1.1); else if (e.wheel.y < 0) camera_scale = std::max(0.05, camera_scale / 1.1);
                    int new_draw_w = (int)std::round(tex_w * camera_scale); int new_draw_h = (int)std::round(tex_h * camera_scale);
                    int cx = (win_w - new_draw_w) / 2; int cy = (win_h - new_draw_h) / 2; pan_x = (int)std::lround((mx - tx * camera_scale) - cx); pan_y = (int)std::lround((my - ty * camera_scale) - cy); recompute_layout(draw_w, draw_h, off_x, off_y);
                } else if (e.type == SDL_MOUSEMOTION) {
                    int mx = e.motion.x, my = e.motion.y; if (panning) { pan_x += (mx - last_mx); pan_y += (my - last_my); last_mx = mx; last_my = my; recompute_layout(draw_w, draw_h, off_x, off_y); }
                    int tx = (int)std::round((mx - off_x) / camera_scale); int ty = (int)std::round((my - off_y) / camera_scale);
                    if (tx >= 0 && tx < tex_w && ty >= 0 && ty < tex_h) { bool er = erasing || (!draw_mode && drawing); if (drawing) { stamp_circle(mask, tx, ty, brush, SDL_MapRGBA(mask->format, 255, 0, 0, er ? 0 : 255)); upload_mask(); } }
                }
            }

            // Render overlay scene on same renderer
            SDL_RenderSetViewport(r, nullptr); SDL_RenderSetClipRect(r, nullptr); SDL_RenderSetScale(r, 1.0f, 1.0f); SDL_RenderSetLogicalSize(r, prev_lw, prev_lh);
            // Dim background
            SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND); SDL_SetRenderDrawColor(r, 0, 0, 0, 140); SDL_Rect dim{0,0,win_w,win_h}; SDL_RenderFillRect(r, &dim);

            SDL_Rect dst{ off_x, off_y, draw_w, draw_h }; SDL_RenderCopy(r, background, nullptr, &dst);

            if (initial_area) {
                const auto [area_minx, area_miny, area_maxx, area_maxy] = initial_area->get_bounds();
                const bool bg_is_area_bb = (tex_w == (area_maxx - area_minx + 1)) && (tex_h == (area_maxy - area_miny + 1));
                const int origin_x = bg_is_area_bb ? area_minx : 0; const int origin_y = bg_is_area_bb ? area_miny : 0;
                SDL_SetRenderDrawColor(r, 0, 200, 255, 180); std::vector<SDL_Point> pts; pts.reserve(initial_area->get_points().size() + 1);
                for (const auto& p : initial_area->get_points()) { int sx = off_x + (int)std::round((p.x - origin_x) * camera_scale); int sy = off_y + (int)std::round((p.y - origin_y) * camera_scale); pts.push_back(SDL_Point{ sx, sy }); }
                if (!pts.empty()) { pts.push_back(pts.front()); SDL_RenderDrawLines(r, pts.data(), (int)pts.size()); }
            }

            SDL_SetTextureAlphaMod(mask_tex, 128); SDL_RenderCopy(r, mask_tex, nullptr, &dst);

            SDL_Color col_draw   = draw_mode ? SDL_Color{80,180,80,255}  : SDL_Color{60,60,60,255};
            SDL_Color col_erase  = !draw_mode? SDL_Color{180,80,80,255}  : SDL_Color{60,60,60,255};
            SDL_Color col_finish = SDL_Color{80,80,200,255};
            SDL_Rect btn_draw{10,10,100,30}; SDL_Rect btn_erase{120,10,100,30}; SDL_Rect btn_save{win_w-120,10,100,30};
            ae_render_button(r, font, btn_draw,  col_draw,   "Draw (D)");
            ae_render_button(r, font, btn_erase, col_erase,  "Erase (E)");
            ae_render_button(r, font, btn_save,  col_finish, "Done (Enter)");
            if (font) {
                SDL_Color hint{220,220,220,255}; const char* txt = "LMB=draw, RMB=erase, MMB=pan, Wheel=zoom, F=fit, 1=1x, +/-=brush, C=clear";
                SDL_Surface* s = TTF_RenderText_Blended(font, txt, hint); if (s) { SDL_Texture* t = SDL_CreateTextureFromSurface(r, s); if (t) { int tw=0, th=0; SDL_QueryTexture(t,nullptr,nullptr,&tw,&th); SDL_Rect dd{10,48,tw,th}; SDL_RenderCopy(r,t,nullptr,&dd); SDL_DestroyTexture(t);} SDL_FreeSurface(s); }
            }

            // Brush preview
            int mx, my; SDL_GetMouseState(&mx, &my); SDL_SetRenderDrawColor(r, 255,255,255,255);
            for (int a = 0; a < 360; a += 12) { double rad = a * M_PI / 180.0; int x = mx + (int)std::cos(rad) * brush; int y = my + (int)std::sin(rad) * brush; SDL_RenderDrawPoint(r, x, y); }

            SDL_RenderPresent(r);
        }

        if (font) { TTF_CloseFont(font); font = nullptr; } if (ttf_inited_here) { TTF_Quit(); }
        EditorResult res; res.bg_w = tex_w; res.bg_h = tex_h; res.points = extract_edge_points(mask, 1);
        SDL_DestroyTexture(mask_tex); SDL_FreeSurface(mask);

        // Restore renderer state
        SDL_SetRenderTarget(r, prev_target); SDL_RenderSetViewport(r, &prev_vp); SDL_RenderSetClipRect(r, &prev_clip); SDL_RenderSetScale(r, prev_sx, prev_sy);
        if (prev_lw > 0 && prev_lh > 0) SDL_RenderSetLogicalSize(r, prev_lw, prev_lh); else SDL_RenderSetLogicalSize(r, 0, 0);
        SDL_RenderSetIntegerScale(r, prev_integer);
    ...
    static EditorResult run_area_editor(SDL_Texture* background,
		...
		SDL_FillRect(mask, nullptr, SDL_MapRGBA(mask->format, 255, 0, 0, 0));
			...
			if (src_renderer) {
				...
				if (tmp) {
						...
						int prev_lw = 0, prev_lh = 0; SDL_RenderGetLogicalSize(src_renderer, &prev_lw, &prev_lh);
						SDL_bool prev_integer = SDL_RenderGetIntegerScale(src_renderer);
						...
						// Disable logical size/integer scaling so copy is exact pixel-for-pixel
						SDL_RenderSetLogicalSize(src_renderer, 0, 0);
						SDL_RenderSetIntegerScale(src_renderer, SDL_FALSE);
					...
					if (bg_surf) {
						...
						if (prev_lw > 0 && prev_lh > 0) SDL_RenderSetLogicalSize(src_renderer, prev_lw, prev_lh); else SDL_RenderSetLogicalSize(src_renderer, 0, 0);
						SDL_RenderSetIntegerScale(src_renderer, prev_integer);
		...
		const double fit_scale = std::min(static_cast<double>(win_w) / tex_w, static_cast<double>(win_h) / tex_h);
		double camera_scale = fit_scale; // Start fully fit to window so entire image is visible
		int pan_x = 0, pan_y = 0;
		auto recompute_layout = [&](int& draw_w, int& draw_h, int& off_x, int& off_y){
			draw_w = static_cast<int>(std::round(tex_w * camera_scale));
			draw_h = static_cast<int>(std::round(tex_h * camera_scale));
		...
		bool quit = false;
		bool draw_mode = true;
		// Initialize font (optional) once
		bool ttf_inited_here = false;
		if (TTF_WasInit() == 0) { if (TTF_Init() == 0) ttf_inited_here = true; }
		TTF_Font* font = nullptr;
		auto open_font = [&]() -> TTF_Font* {
			TTF_Font* f = TTF_OpenFont("C:/Windows/Fonts/consola.ttf", 16);
			if (!f) f = TTF_OpenFont("C:/Windows/Fonts/arial.ttf", 16);
			return f;
		};
		font = open_font();
		while (!quit) {
			...
			if (overlay) {
				...
				// Draw UI buttons with text (Draw / Erase / Done)
			SDL_Color col_draw   = draw_mode ? SDL_Color{80,180,80,255}  : SDL_Color{60,60,60,255};
			SDL_Color col_erase  = !draw_mode? SDL_Color{180,80,80,255}  : SDL_Color{60,60,60,255};
			SDL_Color col_finish = SDL_Color{80,80,200,255};
			SDL_Rect btn_draw{10,10,100,30};
			SDL_Rect btn_erase{120,10,100,30};
			SDL_Rect btn_save{win_w-120,10,100,30};
				ae_render_button(rend, font, btn_draw,  col_draw,   "Draw (D)");
				ae_render_button(rend, font, btn_erase, col_erase,  "Erase (E)");
				ae_render_button(rend, font, btn_save,  col_finish, "Done (Enter)");
			// Hints
			if (font) {
				SDL_Color hint{220,220,220,255};
				const char* txt = "LMB=draw, RMB=erase, MMB=pan, Wheel=zoom, F=fit, 1=1x, +/-=brush, C=clear";
				SDL_Surface* s = TTF_RenderText_Blended(font, txt, hint);
				if (s) {
					SDL_Texture* t = SDL_CreateTextureFromSurface(rend, s);
					if (t) {
						int tw=0, th=0; SDL_QueryTexture(t, nullptr, nullptr, &tw, &th);
						SDL_Rect dd{ 10, 48, tw, th };
						SDL_RenderCopy(rend, t, nullptr, &dd);
...
: area_name_(name)
{
    ...
    EditorResult er = renderer ? run_area_editor_overlay(bg, renderer, window_w, window_h, &base)
                               : run_area_editor(bg, renderer, window_w, window_h, &base);
	if (er.points.empty()) {
...
: area_name_(name)
{
    ...
    EditorResult er = renderer ? run_area_editor_overlay(background, renderer, window_w, window_h, nullptr)
                               : run_area_editor(background, renderer, window_w, window_h, nullptr);
	if (er.points.empty()) {
		throw std::runtime_error("[Area: editor] No points drawn");
	}
	int pivot_x = er.bg_w / 2;
