...
/**
 * camera_grid with realism settings and optional perspective style effects.
 *
...
public:
    ...
    // Keep the camera_grid pitched downward; avoid inversion by capping just shy of 90°.
    ...
    struct RealismSettings {
        ...
        // Depth offset is fixed behind the player in screen pixels (non-user adjustable).
    ...
    // Runtime camera_grid geometry helpers (derived from zoom + offsets).
...
private:
    ...
    SDL_Rect compute_grid_world_rect() const;
    void collect_grid_candidates(world::Grid& world_grid);
    void warp_grid_points(const world::Grid& world_grid);
    void rebuild_active_chunks(const world::Grid& world_grid);
