...
DaveyController::DaveyController(Assets* assets, Asset* player, ActiveAssetsManager& aam)
   : assets_(assets), player_(player), aam_(aam) {}

bool DaveyController::aabb(const Area& A, const Area& B) const {
...
bool DaveyController::pointInAABB(int x, int y, const Area& B) const {
   auto [b_minx, b_miny, b_maxx, b_maxy] = B.get_bounds();
   return (x >= b_minx && x <= b_maxx &&
           y >= b_miny && y <= b_maxy);
}

void DaveyController::movement(const Input& input) {
   dx_ = dy_ = 0;
   if (!player_) return;
...
bool DaveyController::canMove(int offset_x, int offset_y) {
...
void DaveyController::interaction() {
...
void DaveyController::handle_teleport(const Input& input) {
   if (!player_) {
      std::cerr << "[DaveyController::handle_teleport] player_ null\n";
      return;
   }
...
int DaveyController::get_dx() const { return dx_; }
int DaveyController::get_dy() const { return dy_; }
