#include "percentage_spawner.hpp"
...
void PercentageSpawner::spawn(const SpawnInfo& item, const Area* area, SpawnContext& ctx) {
    ...
    if (ctx.checker().check(item.info, final_x, final_y, ctx.exclusion_zones(), ctx.all_assets(),
        ...
        ctx.logger().output_and_log(item.name, item.quantity, 0, 0, 0, "percentage");
        return;
    }

    ctx.spawnAsset(item.name, item.info, "percentage", final_x, final_y, 0, nullptr, item.asset_id);
    ctx.logger().progress(item.info, 1, item.quantity);
    ctx.logger().output_and_log(item.name, item.quantity, 1, 1, 1, "percentage");
}
