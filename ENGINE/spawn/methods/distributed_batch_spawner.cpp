...
void DistributedBatchSpawner::spawn(const std::vector<BatchSpawnInfo>& items,
    ...
    // Shuffle to distribute batch picks spatially
    ...
    for (auto* gp : points) {
        ...
        if (selected.name == "null") {
            // Reserve this grid point as empty space
        ...
        auto& info = it->second;
        if (ctx.checker().check(info, pos, ctx.exclusion_zones(), ctx.all_assets(), true, false, true, 5)) continue;
        auto* result = ctx.spawnAsset(selected.name, info, *area, pos, 0, nullptr, spawn_id, std::string("DistributedBatch"));
        if (result) {
            ctx.grid()->set_occupied(gp, true);
