...
private:
    ...
    void gather_from_cells(const std::unordered_map<CellKey, std::vector<Asset*>>& cells,
                           SDL_Point pos,
                           int radius,
                           std::vector<Asset*>& out,
                           std::unordered_set<const Asset*>& seen) const;

    void gather_from_named_cells(const std::unordered_map<std::string, std::unordered_map<CellKey, std::vector<Asset*>>>& cells,
    ...
    bool perform_spacing_checks_grid(const std::shared_ptr<AssetInfo>& info,
                                     const SDL_Point& test_pos,
                                     bool enforce_spacing_for_candidate,
                                     bool treat_as_edge_asset,
                                     bool treat_as_map_asset) const;

    bool perform_spacing_checks_linear(const std::shared_ptr<AssetInfo>& info,
                                       const SDL_Point& test_pos,
                                       const std::vector<std::unique_ptr<Asset>>& assets,
                                       bool enforce_spacing_for_candidate,
                                       bool treat_as_edge_asset,
