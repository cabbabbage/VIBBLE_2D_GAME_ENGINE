...
import pandas as pd
from asset_pages.animation_uploader import AnimationUploader
...
class AnimationEditor(ttk.Frame):
    ...
    def _trigger_autosave(self):
        if callable(self.on_change):
            self.on_change()
        try:
            if self.asset_folder and self.trigger_name and self.area_ui and self.area_ui.area_data:
                info_path = os.path.join(self.asset_folder, "info.json")
                info = load_info(info_path)
                info.setdefault("animations", {})
                anim = info["animations"].setdefault(self.trigger_name, {})
                anim["area"] = self.area_ui.area_data
